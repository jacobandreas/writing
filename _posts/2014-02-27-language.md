...
1. Again, think of the underlying representation of meaning as a POMDP. What
   does language do in this world?
   1. Let's limit ourselves to a literal listener. This means declaratives and
      imperatives, but no questions.
   1. It would be formally convenient if language only talked about the
      probability of being in states.
   1. Let our _state_ representation encode the action currently executed by
      every entity in the world.
   1. "I am eating truffles"---sharpen my belief around states where this is
      true.
   1. "I will eat truffles tomorrow"---increase the posterior of paths including
      that (single) edge; that is, increase transition probabilities into
      truffle-eating states.
   1. "I will buy truffles every Thursday"---increase transition probabilities
      along all such edges.
   1. "I might buy truffles tomorrow"---as above, but less so.
   1. "You must buy truffles tomorrow"---increase an estimate of _value_.
   1. We could do many of the above things by altering single transition
      probabilities, rather than all transitions in or out of a state.
   1. We could probably replace _value_ with _reward_ above.
   1. An agent's behavior in an MDP is (over)specified by a model (which is a
   ...
   1. If everything is covered in here somewhere, we don't need to worry about
      cleanly separating these updates according to pre-existing linguistic
      theory.
   1. Because all states remember their history, this has the form of a
      (minimax) tree search rather than a graph search.
   1. This makes it easy (formally, not computationally) to incorporate
...
1. So: at time $$t$$ I maintain a belief over current states in the
   ...
   1. My behavior, in turn, is specified by a (heuristic!) estimate of the value
      of every state.
   1. We can say: declarative sentences increase or decrease the probability
      of individual paths in this tree (i.e. my beliefs).
   1. We can say: imperative sentences increase or decrease my estimates of the
      value along individual paths in this tree.
   1. So the meaning of a sentence is: 1. an update type (probably just from
